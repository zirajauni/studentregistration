...
if(isset($_POST['submit']))
{
	...
	$check_user=("SELECT id FROM `registration` WHERE (username='$username' AND pword='$pword')");

	$run_user=mysqli_query($conn, $check_user);

	
	if (mysqli_num_rows($run_user)){
		echo "welcome";  
	...
	else
	{
		...
		//header("Location: ../signin pages/login.php"); 
...
<div id="skipnav"><a href="#maincontent">Skip to main content</a></div>
    ...
    <header>
        <div class="container" id="maincontent" tabindex="-1">
            <div class="row">
                <div class="col-lg-12">
                <h2 class="f">Please sign in</h2>
                  <form action="signin.php" method="post">
			        <label for="" class="sr-only">username</label>
			        <input type="text" id="username" name="username" class="form-control" placeholder="username" required autofocus>
					<label for="inputPassword" class="sr-only">Password</label>
			        <input type="password" id="pword"  name="pword" class="form-control" placeholder="pword" required>
			        <div class="checkbox">
			          <label>
